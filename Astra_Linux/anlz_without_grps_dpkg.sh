...
get_package_info() {
    ...
    # Для dpkg используем dpkg -S для поиска пакета по файлу
    local package=$(dpkg -S "$filepath" 2>/dev/null | cut -d':' -f1 | head -1)
    
    if [ -n "$package" ]; then
        # Получаем информацию о пакете через dpkg -s
        # В deb пакетах нет прямого аналога группы, используем Section
        local section=$(dpkg -s "$package" 2>/dev/null | grep -i "^Section" | cut -d':' -f2- | sed 's/^[ \t]*//' | head -1)
...
print_table() {
    ...
    printf "${BOLD}%-${w_relro}s %-${w_canary}s %-${w_nx}s %-${w_pie}s %-${w_rpath}s %-${w_runpath}s %-${w_symbols}s %-${w_fortify}s %-${w_fortified}s %-${w_fortifiable}s %-${w_packet}s %-${w_group}s %s${NC}\n" \
           "RELRO" "STACK CANARY" "NX" "PIE" "RPATH" "RUNPATH" "Symbols" "FORTIFY" "Fortified" "Fortifiable" "Packet" "Section" "Filename"
...
main() {
    ...
    echo -e "\n${BOLD}Получение информации о пакетах и секциях (dpkg)...${NC}"
